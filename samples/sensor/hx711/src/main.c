...
void set_rate(enum hx711_rate rate)
{
	...
	rate_val.val1 = rate;
	sensor_attr_set(hx711, SENSOR_CHAN_WEIGHT, SENSOR_ATTR_SAMPLING_FREQUENCY, &rate_val);
}
...
int main(void)
{
	...
	sensor_attr_set(hx711, SENSOR_CHAN_WEIGHT, SENSOR_ATTR_CALIBRATION, &calibration_weight);

	LOG_INF("waiting for known weight of %d.%06d grams...", calibration_weight.val1,

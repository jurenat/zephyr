...
static int hx711_attr_set_gain(const struct device *dev, const struct sensor_value *val)
{
	...
	if (!IN_RANGE(val->val1, HX711_GAIN_128X, HX711_GAIN_64X)) {
		return -ENOTSUP;
	}
...
static int hx711_bulk_read(const struct device *dev, uint8_t *readings, uint32_t *sum)
{
	struct hx711_data *data = dev->data;
...
int avia_hx711_tare(const struct device *dev, uint8_t readings)
{
	...
	ret = hx711_bulk_read(dev, &readings, &avg);
	if (ret) {
		return ret;
	}

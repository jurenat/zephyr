...
static void ina230_handle_interrupts(const struct device *dev)
{
	...
	if (ina23x_reg_read_16(&config->bus, INA230_REG_MASK, &reg) < 0) {
		return;
	}

	if (data->handler_alert && reg & reg & INA230_ALERT_FUNCTION_FLAG) {
...
static K_KERNEL_STACK_DEFINE(ina230_thread_stack, CONFIG_INA230_THREAD_STACK_SIZE);
static struct k_thread ina230_thread;
		       ...
		       sensor_trigger_handler_t handler)
{
	...
	if (ret) {
		LOG_ERR("Failed to enable CNVR flag");
		goto exit;
	}
...
int ina230_trigger_mode_init(const struct device *dev)
{
	...
	k_tid_t tid =
		k_thread_create(&ina230_thread, ina230_thread_stack,
